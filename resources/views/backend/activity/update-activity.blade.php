...
@section('content')
    ...
    <div class="content-wrapper">
        <div class="container-xxl flex-grow-1 container-p-y">
            <div class="col-xl-12">
                <!-- File input -->
                <form action="{{ route('submit.edit.activities') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{ $news->id }}">

                    <!-- Title -->
                    <div class="mb-3">
                        <label class="form-label text-[#0F4634]">Title English</label>
                        <input type="text" name="edit_title" value="{{ $news->title }}" class="form-control">
                    </div>

                    <!-- Description -->
                    <div class="mb-3">
                        <label class="form-label text-[#0F4634]">Description English</label>
                        <textarea name="edit_description" class="form-control" rows="5">{{ $news->description }}</textarea>
                    </div>
                    <!-- Title -->
                    <div class="mb-3">
                        <label class="form-label text-[#0F4634]">Title Khmer</label>
                        <input type="text" name="edit_title_km" value="{{ $news->title_km }}" class="form-control">
                    </div>

                    <!-- Description -->
                    <div class="mb-3">
                        <label class="form-label text-[#0F4634]">Description Khmer</label>
                        <textarea name="edit_description_km" class="form-control" rows="5">{{ $news->description_km }}</textarea>
                    </div>
