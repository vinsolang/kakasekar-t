...
<body class="overflow-x-hidden">
    <section id="team-page" class="relative w-full bg-[#FFFFFF]" style="background: #FFFFFF;">
        ...
        <header class="absolute top-0 left-0 w-full z-50">
            <!-- Top Info Bar -->
            <div class="lg:flex hidden flex-col-3 lg:flex-row justify-end lg:space-x-4 lg:space-x-6 items-center h-auto lg:h-[54px] 
                ...
                <div class="flex items-center xl:space-x-10 space-x-2">
                    ...
                    <nav class="hidden lg:flex text-[#1E3E0F] font-medium items-center">
                        <ul class="flex lg:flex-wrap justify-center gap-4 space-x-4 lg:gap-2 xl:gap-4 text-[15px] lg:text-[16px] items-center">
                            ...
                            <li class="relative group mt-[-10px]">
                                <a href="{{ route('activities') }}" class="px-4 lg:w-[110px] lg:h-[98px] flex items-center justify-center 
    ...
    shadow-md bg-white overflow-y-auto mt-[70px]">
        ...
        <div class="relative w-full min-h-[600px] md:min-h-[800px] lg:min-h-[1000px] flex flex-col justify-center items-center bg-cover bg-center text-white"
            ...
            <!-- Description -->
            <div class="mt-8 sm:mt-10 md:mt-12 w-11/12 sm:w-4/5 md:w-[700px] lg:w-[900px] text-left px-4">
        ...
        <section class="py-8 px-4 text-center bg-transparent">
            <p class="text-[#1E3E0F] text-sm sm:text-base md:text-base lg:text-base mx-auto">
                 @lang('messages.a2')
            </p>
        </section>
    ...
    <section id="card-news" class="relative py-16 sm:py-20">
        <div class="container mx-auto flex flex-col gap-16 px-4 sm:px-6 lg:px-8">
            ...
            @foreach($newsList as $index => $news)
                <div
                    class="flex flex-col lg:flex-row {{ $index % 2 == 1 ? 'lg:flex-row-reverse' : '' }} items-center gap-8 lg:gap-12">
...
@endphp
                    ...
                    <div class="w-full lg:w-1/2 max-w-xl text-center lg:text-left">
                         <p class="text-base sm:text-lg md:text-xl font-medium mb-6 leading-relaxed text-[#000000] text-clamp-4">
                            {{ app()->getLocale() === 'km' ? Str::limit($news->title_km, 150) : Str::limit($news->title, 150) }}
                        </p>
                        {{-- <div class="text-center lg:text-left">
                            <button 
                                class="read-detail-btn w-[160px] sm:w-[200px] h-[44px] sm:h-[50px] md:w-[214px] md:h-[54px]
                        ...
                        <button 
                            class="read-detail-btn bg-gradient-to-r from-[#4FA428] to-[#1E3E0F] text-white rounded-full px-6 py-2 w-[214px] h-[54px]"
                            data-id="{{ $news->id }}"
                            data-description="{{ app()->getLocale() === 'km' ? $news->description_km : $news->description }}"
                            data-images='@json($images)'>
                            @lang('messages.read_detail')
                        </button>
                    </div>
                </div>
            @endforeach
    ...
    {{-- News Detail --}}
    <section id="cms-news" class="hidden w-full max-w-[1373px] mx-auto">
    <div class="news flex flex-col gap-4 relative" id="news-detail-content">
        <!-- JS will inject content here -->
    </div>

    <div class="mt-6">
        <button id="back-btn" class="read-detail-btn bg-gradient-to-r from-[#4FA428] to-[#1E3E0F] text-white rounded-full px-6 py-2">
            @lang('messages.back')
        </button>
    </div>
</section>
    ...
    <section id="innovation-nature"
        class="w-full h-[553px] bg-[url('{{ asset('image/innovation.png') }}')] bg-cover bg-center flex items-center justify-center text-white relative mt-[80px]">
        <!-- Overlay -->
        <div
...
<script>
    ...
    // Button read detail news
    document.addEventListener("DOMContentLoaded", () => {
    // All "Read Detail" buttons
    const detailButtons = document.querySelectorAll(".read-detail-btn");
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // fade/slide effect classes
    cardNews.classList.add("fade", "fade-show");
    cmsNews.classList.add("fade", "fade-hidden");

    detailButtons.forEach((btn, index) => {
        btn.addEventListener("click", () => {
            // pass index of clicked news
            showDetail(index);
        });
    });
    ...
    function showDetail(index) {
        // hide list
        cardNews.classList.replace("fade-show", "fade-hidden");
        setTimeout(() => {
            cmsNews.classList.replace("fade-hidden", "fade-show");
        }, 300);
    }
    ...
    if (backBtn) {
        backBtn.addEventListener("click", () => {
            cmsNews.classList.replace("fade-show", "fade-hidden");
            setTimeout(() => {
                cardNews.classList.replace("fade-hidden", "fade-show");
            }, 300);
        });
    }
});
// Show News more detail
document.addEventListener("DOMContentLoaded", () => {
    const cardNews = document.getElementById("card-news");
    const cmsNews = document.getElementById("cms-news");
    ...
    detailButtons.forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            const img = (i) => images[i] ? `<img src="/${images[i]}" class="w-full h-full object-cover rounded-xl aspect-[16/9]">` : '';

            // Build detail layout (same as yours, but only show if image exists)
            let layout = `
                <div class="space-y-6">

                    <!-- Block 1 -->
                    ${(images[1] || images[2]) ? `
                    <div class="news-block-1 flex flex-col lg:flex-row gap-4">
                        <div class="block-1 w-full lg:w-[70%]">
                            ${img(1)}
                        </div>
                        <div class="block-2 w-full lg:w-[30%]">
                            ${img(2)}
                        </div>
                    </div>
                    ` : ''}
                    ...
                    <!-- Block 3 -->
                    ${(images[3] || images[4]) ? `
                    <div class="news-block-3 flex flex-col sm:flex-row gap-4">
                        <div class="block-1 w-full sm:w-1/2">
                            ${img(3)}
                        </div>
                        <div class="block-2 w-full sm:w-1/2">
                            ${img(4)}
                        </div>
                    </div>
                    ` : ''}
                    ...
                    <!-- Block 4 -->
                    ${(images[5] || images[6]) ? `
                    <div class="news-block-4 flex flex-col lg:flex-row gap-4">
                        <div class="block-1 w-full lg:w-[30%]">
                            ${img(5)}
                        </div>
                        <div class="block-2 w-full lg:w-[70%]">
                            ${img(6)}
                        </div>
                    </div>
                    ` : ''}
            ...
            detailContent.innerHTML = layout;
    ...
    backBtn.addEventListener("click", () => {
        cmsNews.classList.add("hidden");
        cardNews.classList.remove("hidden");
    });
});
</script>
