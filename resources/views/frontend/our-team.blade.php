...
<head>
    ...
    <title>@lang('messages.our_team')</title>
    {{-- Link Icon --}}
...
<body class="overflow-x-hidden">
                ...
                <div class="flex items-center xl:space-x-10 space-x-2">
                    ...
                    <nav class="hidden lg:flex text-[#1E3E0F] font-medium items-center">
                        <ul class="flex flex-wrap justify-center gap-4 space-x-4 lg:gap-4 xl:gap-6 text-[15px] lg:text-[16px] items-center">
                            ...
                            <li class="mt-[-10px]"><a href="{{ route('team') }}" class="px-4 lg:w-[106px] lg:h-[98px] flex items-center justify-center 
                        rounded-bl-[12px] rounded-tr-[12px] 
                        bg-gradient-to-b from-[#4FA428] to-[#1E3E0F] 
                        text-white font-medium">@lang('messages.our_team')</a></li>
    ...
    shadow-md bg-white overflow-y-auto mt-[70px]">
        ...
        <!-- Hero Section -->
        <div class="relative w-full h-[1000px] flex flex-col justify-center items-center bg-cover bg-center text-white"
            style="background-image: url('{{ asset('image/team.jpg') }}');">
            <h1 class="text-[60px] md:text-[100px] font-bold">@lang('messages.our_team')</h1>
            <div class="w-[154px] h-[4px] bg-white mt-4"></div>
        ...
        <section class="py-12 px-4 text-center bg-transparent shadow-none">
            <p class="text-[#1E3E0F] text-[15px] line-clamp-2 text-left flex justify-center">
                @lang('messages.title_team')
            </p>
        </section>

        <!-- Team Profiles -->
        <section class="py-12 px-4 relative z-10 bg-white">
            <div class="max-w-6xl mx-auto space-y-20">
                @foreach ($showTeam as $team)
                    <div class="flex flex-col md:flex-row items-stretch gap-10">

                        <!-- Profile Image with Dark Green Background -->
                        <div
                            class="flex-shrink-0 flex items-center justify-center mt-[45px] transform transition-transform duration-300 hover:scale-110">
                            <div
                                class="relative w-[230px] h-[240px] rounded-full bg-[#1E3E0F] flex items-center justify-center">
                                <img src="../../assets/team/{{ $team->profile }}" alt="{{ $team->name }}"
                                    class="w-[220px] h-[280px] object-cover rounded-full absolute top-[-40px]">
                            </div>
                        </div>

                        <!-- Text Content (match height with flex-1 + flex) -->
                        <div class="flex-1 flex flex-col justify-center">
                            <h1 class="text-[28px] font-bold text-[#1E3E0F]">
                                {{ app()->getLocale() === 'km' ? $team->name_km : $team->name }}
                            </h1>

                            <h2 class="text-[22px] font-medium text-[#1E3E0F] mt-1">
                                {{ app()->getLocale() === 'km' ? $team->position_km : $team->position }}
                            </h2>

                            <div class="w-full h-[2px] bg-[#0F4634] my-4"></div>

                            <p class="text-[#1E3E0F] text-[15px] leading-relaxed text-justify">
                                {{ app()->getLocale() === 'km' ? $team->description_km : $team->description }}
                            </p>
                        </div>
                    </div>
                @endforeach
    ...
    <section id="innovation-nature"
        class="w-full h-[553px] bg-[url('{{ asset('image/innovation.png') }}')] bg-cover bg-center flex items-center justify-center text-white relative mt-[50px]">
        <!-- Overlay -->
        <div
